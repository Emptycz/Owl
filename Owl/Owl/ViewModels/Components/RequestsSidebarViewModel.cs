...
public partial class RequestsSidebarViewModel : ViewModelBase
{
    ...
    public RequestsSidebarViewModel()
    {
        _environmentRepository = App.Current?.Services?.GetRequiredService<IEnvironmentRepository>() ?? throw new InvalidOperationException(
            "EnvironmentRepository is not registered in the service provider.");
        ...
        _repository = App.Current?.Services?.GetRequiredService<IRequestNodeRepository>() ?? throw new InvalidOperationException(
        ...
        _dbContext = App.Current?.Services?.GetRequiredService<IDbContext>() ?? throw new InvalidOperationException(
            "DbContext is not registered in the service provider.");
    }
